...
pander(summary(breach_df$Individuals.Affected), caption="Individuals Affected - All")

pander(summary(breach_df$Individuals.Affected.Normalized.Outliers), caption="Individuals Affected - Normalized")
```

-----

```{r, fig.width=10}
# Boxplot of Individuals
...
kable(summary_table(group_by(breach_df, Year), summary_data), caption='Individuals Affected - Summary (Yearly)', align = "l")
```
...
kable(summary_table(group_by(breach_df, Month), summary_data), caption='Individuals Affected - Summary (Monthly)', longtable = TRUE, align = "l")
...
plot_ly() %>%
  add_trace(x = fit.1$x, y=fit.1$y, mode = "lines", fill = "tozeroy", yaxis = "y2", name = "Random Sampling Without Replacement") %>% 
  add_trace(x = fit.2$x, y=fit.2$y, mode = "lines", fill = "tozeroy", yaxis = "y2", name = "Systematic Sampling") %>% 
  add_trace(x = fit.3$x, y=fit.3$y, mode = "lines", fill = "tozeroy", yaxis = "y2", name = "Inclusion Probabilities") %>% 
...
plot_ly(x, y=x$Name.of.Covered.Entity, x=x$n, orientation = 'h', type='bar') %>%
  ...
  layout(title = "Top 5 Breach Targets",
         xaxis = list(title = "Number of Breaches"),
         yaxis = list(title = "")) 
